  ...
  Selection: TableSelection,
{
  ...
  fn render_iter<'b>(
    ...
    // horizontal layout
    let (width, l_columns, l_spacers) =
      self.layout_columns(state.table_area.width);
        ...
        if render_row_area.height > 0 {
          ...
          loop {
            ...
            if render_cell_area.right() > state.hscroll.offset as u16
              || render_cell_area.left()
                < state.hscroll.offset as u16 + area.width
            {
              if self.auto_styles {
  ...
  fn calculate_column_areas(
    ...
    loop {
      ...
      let cell_x2 = (l_columns[col].x
        + l_columns[col].width
        + l_spacers[col + 1].width) as isize;
...
impl<Selection> TableState<Selection> {
  ...
  pub fn row_cells(&self, row: usize) -> Option<(Rect, Vec<Rect>)> {
    if row < self.vscroll.offset()
      || row >= self.vscroll.offset() + self.vscroll.page_len()
    {
      return None;
    }
...
impl<Selection: TableSelection> TableState<Selection> {
  ...
  pub fn scroll_to_row(&mut self, pos: usize) -> bool {
    if pos >= self.rows {
      ...
      if heights < self.table_area.height {
        ...
        self
          .set_row_offset(pos.saturating_sub(self.page_len()).saturating_add(1))
      }
    } else if pos >= self.row_offset().saturating_add(self.page_len()) {
  ...
  pub fn scroll_to_col(&mut self, pos: usize) -> bool {
    if let Some(col) = self.column_layout.get(pos) {
      ...
      } else if (col.right() as usize)
        >= self.x_offset().saturating_add(self.page_width())
      {
        self.set_x_offset(
          (col.right() as usize).saturating_sub(self.page_width()),
        )
      } else {
        false
      }
  ...
  fn handle(
    ...
    match event {
      ct_event!(mouse any for m)
        if self.mouse.doubleclick(self.table_area, m) =>
      {
