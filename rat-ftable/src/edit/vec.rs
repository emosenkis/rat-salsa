...
use rat_event::{
  HandleEvent, Outcome, Regular, ct_event, event_flow, try_flow,
};
    ...
    if self.mode == Mode::Edit || self.mode == Mode::Insert {
      if self.is_focused() {
        ...
        try_flow!(match event {
          ...
          ct_event!(keycode press Enter) => {
            if self.table.selected_checked()
              < Some(self.table.rows().saturating_sub(1))
            {
      ...
      try_flow!(match event {
        ct_event!(mouse any for m)
          if self.mouse.doubleclick(self.table.table_area, m) =>
        {
          if let Some((col, row)) =
            self.table.cell_at_clicked((m.column, m.row))
          {
