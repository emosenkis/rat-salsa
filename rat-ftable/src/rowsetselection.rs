...
impl RowSetSelection {
  ...
  pub fn move_up(&mut self, n: usize, maximum: usize, extend: bool) -> bool {
    ...
    self.extend(extend);
    self.lead_row =
      Some(self.lead_row.map_or(maximum, |v| v.saturating_sub(n)));
    old_selection != (self.anchor_row, self.lead_row)
  }
...
impl HandleEvent<crossterm::event::Event, Regular, TableOutcome>
  for TableState<RowSetSelection>
{
  fn handle(
    &mut self,
...
impl HandleEvent<crossterm::event::Event, MouseOnly, TableOutcome>
  for TableState<RowSetSelection>
{
  fn handle(
    &mut self,
