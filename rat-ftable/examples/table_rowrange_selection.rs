...
fn main() -> Result<(), anyhow::Error> {
  ...
  run_ui("rowrange_selection", mock_init, event, render, &mut state)
}
...
struct State {
  ...
  table: TableState<RowSetSelection>,
}
...
fn event(
  ...
  let r = rowsetselection::handle_events(&mut state.table, true, event);
  Ok(r.into())
}
