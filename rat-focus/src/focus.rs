...
impl Focus {
  ...
  pub fn next(&self) -> bool {
    match self.navigation() {
      ...
      Some(
        Navigation::Leave | Navigation::ReachLeaveBack | Navigation::Regular,
      ) => {
        focus_debug!(
          self.core,
          "next after {:?}",
  ...
  pub fn prev(&self) -> bool {
    match self.navigation() {
      ...
      Some(
        Navigation::Leave | Navigation::ReachLeaveFront | Navigation::Regular,
      ) => {
        focus_debug!(
          self.core,
          "prev before {:?}",
...
mod core {
  ...
  impl FocusCore {
    ...
    pub(super) fn remove_container(&mut self, cidx: usize) -> FocusCore {
      ...
      // remove
      let focus_flags =
        self.focus_flags.drain(crange.clone()).collect::<Vec<_>>();
      ...
      let mut sub_container_ids: HashSet<usize, FxBuildHasher> =
        HashSet::default();
    ...
    fn __accumulate(&self) {
      ...
      for (f, r) in &self.containers {
        ...
        if any_gained && !any_lost {
          if let Some(on_gained_cb) =
            f.container_flag.0.on_gained.borrow().as_ref()
          {
            focus_debug!(
              self,
    ...
    fn test_insert_remove() {
      ...
      impl HasFocus for DD {
        fn build(&self, fb: &mut FocusBuilder) {
          let tag =
            fb.start_with_flags(self.dd.clone(), self.area(), self.area_z());
          fb.widget(&self.g);
          fb.widget(&self.h);
          fb.widget(&self.i);
          fb.end(tag);
