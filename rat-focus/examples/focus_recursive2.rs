...
fn render(
  ...
  let l11 = Layout::vertical([Constraint::Length(8), Constraint::Length(8)])
    .split(l0[1]);
...
pub mod substratum2 {
  ...
  impl<'a> StatefulWidget for Substratum2<'a> {
    ...
    fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
      ...
      let ll = Layout::vertical([Constraint::Length(8), Constraint::Length(8)])
        .split(inner);

      let ss1 =
        Substratum::new(self.theme).block(Block::bordered().title("Primus"));
      ss1.render(ll[0], buf, &mut state.stratum1);

      let ss2 =
        Substratum::new(self.theme).block(Block::bordered().title("Secundus"));
