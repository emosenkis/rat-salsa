...
use ratatui::symbols;
use ratatui::widgets::{
  Padding, Scrollbar, ScrollbarOrientation, ScrollbarState, StatefulWidget,
};
use std::cmp::{max, min};
use std::mem;
use std::ops::Range;
  ...
  fn update_symbols(&mut self) {
    if self.is_horizontal() {
      if let Some(horizontal) = &self.hor_symbols {
        ...
        self.scrollbar =
          mem::take(&mut self.scrollbar).symbols(horizontal.into());
      }
    } else {
      if let Some(vertical) = &self.ver_symbols {
        self.min_symbol = Some(vertical.min);
        self.scrollbar =
          mem::take(&mut self.scrollbar).symbols(vertical.into());
      }
    }
  }
