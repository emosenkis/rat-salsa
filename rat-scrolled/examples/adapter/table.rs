...
use ratatui::widgets::{
  Block, HighlightSpacing, Row, StatefulWidget, Table, TableState,
};
use std::fmt::Debug;
      ...
      if let Some(selected) = self.widget.selected() {
        if selected < new_offset {
          ...
          *self.widget.selected_mut() =
            Some(new_offset + self.scroll.page_len());
        }
      }
  ...
  pub fn scroll_to_selected(&mut self) {
    if let Some(selected) = self.selected() {
      if selected >= self.scroll.offset() + self.scroll.page_len() {
        self
          .set_offset(selected.saturating_sub(self.scroll.page_len() * 9 / 10));
      }
      if selected < self.scroll.offset() {
        self
          .set_offset(selected.saturating_sub(self.scroll.page_len() * 1 / 10));
      }
    }
  }
}
