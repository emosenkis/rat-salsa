...
use rat_widget::statusline_stacked::{
  SLANT_BL_TR, SLANT_TL_BR, StatusLineStacked,
};
...
impl<'a> StatefulWidget for VIStatusLine<'a> {
  ...
  fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
    ...
    if !state.1.matches.list.is_empty() {
      ...
      status = status.end(
        Span::from(format!("{}/{}", idx, state.1.matches.list.len()))
          .style(self.style),
    ...
    if !state.1.finds.list.is_empty() {
      ...
      status = status.end(
        Span::from(format!("🖛 {}/{}", idx, state.1.finds.list.len()))
          .style(self.style),
        Span::from("|").style(self.style),
      );
    }
