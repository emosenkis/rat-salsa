...
fn execute_normal(
  ...
  match vim {
    ...
    Vim::PasteClipboard(mul, before) => {
      paste_clipboard_text(*mul, *before, state, vi)
    }
    Vim::Delete(mul, m) => {
    ...
    let r = if vi.mode == Mode::Normal {
      match event {
        ...
        ct_event!(key press CONTROL-cc) => {
          eval_normal(ctrl::ctrl(*cc), self, vi)?
        }
      ...
      match event {
        ...
        ct_event!(key press CONTROL-cc) => {
          eval_visual(ctrl::ctrl(*cc), self, vi)?
        }
