...
fn render_items(
  ...
  let highlight_style =
    style.patch(widget.highlight_style.unwrap_or(Style::new().underlined()));
  ...
  let sel_right_style =
    focus_style.patch(widget.right_style.unwrap_or_default());
  let sel_highlight_style = focus_style;
  let sel_disabled_style =
  ...
  for (n, item) in widget.menu.items.iter().enumerate() {
    ...
    let item_line = if let Some(highlight) = item.highlight.clone() {
      Line::from_iter([
        ...
        Span::from(&item.item[highlight.start..highlight.end])
...
impl HandleEvent<crossterm::event::Event, Popup, MenuOutcome>
  for PopupMenuState
{
  /// Handle all events.
  ///
...
impl HandleEvent<crossterm::event::Event, MouseOnly, MenuOutcome>
  for PopupMenuState
{
  fn handle(
    &mut self,
