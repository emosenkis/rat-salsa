...
impl Colors {
  ...
  pub fn array() -> &'static [Colors] {
    ...
    // don't include None!
    &[
      TextLight, TextDark, Primary, Secondary, White, Black, Gray, Red, Orange,
      Yellow, LimeGreen, Green, BlueGreen, Cyan, Blue, DeepBlue, Purple,
      Magenta, RedPink,
    ]
  }
...
impl<'de> Visitor<'de> for PaletteVisitor {
    ...
    A: MapAccess<'de>,
  {
    ...
    while let Some(key) = map.next_key::<&str>()? {
      match key {
        "theme_name" => {
          pal.theme_name = map.next_value::<Cow<'static, str>>()?
        }
        ...
        c_str => {
          let cn = Colors::from_str(c_str)
            .map_err(|_| A::Error::unknown_field(c_str, &[]))?;
          if pal.generator.starts_with("light-dark") {
            let mut dark = 63;
            if let Some(s) = pal.generator.split(':').nth(1) {
              dark = s.trim().parse::<u8>().unwrap_or(63);
            ...
            let (c0, c1, c2, c3) =
              map.next_value::<(Color, Color, Color, Color)>()?;
            pal.color[cn as usize][0] = c0;
            pal.color[cn as usize][1] = c1;
            pal.color[cn as usize][2] = c2;
            pal.color[cn as usize][3] = c3;
...
impl Palette {
  ...
  pub fn add_aliased(&mut self, id: &str, color_idx: ColorIdx) {
    ...
    match &mut self.aliased {
      ...
      Cow::Owned(aliased) => {
        match aliased.binary_search_by_key(&id, |v| v.0.as_ref()) {
...
impl Palette {
  ...
  pub fn high_contrast(&self, color: Color) -> Style {
    match Self::rate_text_color(color) {
      None => Style::new(),
      Some(Rating::Light) => {
        Style::new().bg(color).fg(self.color(Colors::TextLight, 3))
      }
      Some(Rating::Dark) => {
        Style::new().bg(color).fg(self.color(Colors::TextDark, 3))
      }
    }
  }
  ...
  pub fn normal_contrast(&self, color: Color) -> Style {
    match Self::rate_text_color(color) {
      None => Style::new(),
      Some(Rating::Light) => {
        Style::new().bg(color).fg(self.color(Colors::TextLight, 0))
      }
      Some(Rating::Dark) => {
        Style::new().bg(color).fg(self.color(Colors::TextDark, 0))
      }
    }
  }
