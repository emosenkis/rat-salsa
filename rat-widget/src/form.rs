  ...
  W: Eq + Hash + Clone,
{
  ...
  pub fn into_buffer<'b, 's>(
    ...
    assert!(
      page_size.height < u16::MAX
        || page_size.height == u16::MAX && state.page == 0
    );
  ...
  fn render_navigation(
    ...
    if !state.layout.is_endless() {
      ...
      if (state.page + 1) < page_count {
        ...
        state.next_area =
          Rect::new(area.x + area.width.saturating_sub(p), area.y, p, 1);
      } else {
...
impl<W> HandleEvent<crossterm::event::Event, Regular, FormOutcome>
  for FormState<W>
where
  W: Eq + Hash + Clone,
{
...
impl<W> HandleEvent<crossterm::event::Event, MouseOnly, FormOutcome>
  for FormState<W>
where
  W: Eq + Hash + Clone,
{
    ...
    if !self.layout.is_endless() {
      match event {
        ct_event!(mouse down Left for x,y)
          if self.prev_area.contains((*x, *y).into()) =>
        {
          if self.prev_page() {
            FormOutcome::Page
          } else {
        ...
        ct_event!(mouse down Left for x,y)
          if self.next_area.contains((*x, *y).into()) =>
        {
          if self.next_page() {
            FormOutcome::Page
          } else {
