...
use ratatui::widgets::{
  Block, HighlightSpacing, ListDirection, ListItem, StatefulWidget,
};
use std::cmp::min;
...
fn render_list<Selection: ListSelection>(
  ...
  let mut item_area =
    Rect::new(state.inner.x, state.inner.y, state.inner.width, 1);
  ...
  if total_height < state.inner.height {
    state.scroll.set_page_len(
      state.row_areas.len() + state.inner.height as usize
...
impl<Selection: ListSelection> ListState<Selection> {
  ...
  pub fn row_area(&self, row: usize) -> Option<Rect> {
    if row < self.scroll.offset()
      || row >= self.scroll.offset() + self.scroll.page_len()
    {
      return None;
    }
    ...
    fn handle(
      ...
      let res = if self.is_focused() {
        match event {
          ...
          | ct_event!(keycode press Home) => self.scroll_to(0).into(),
    ...
    fn handle(
      ...
      event_flow!(
        return match event {
          ...
          {
            self
              .move_to(self.row_at_drag((m.column, m.row)), true)
              .into()
          }
          ct_event!(mouse down Left for column, row) => {
            let pos = (*column, *row);
