...
use rat_event::{
  ConsumedEvent, HandleEvent, MouseOnly, Outcome, Regular, ct_event,
};
use rat_focus::{Focus, FocusBuilder, FocusFlag, HasFocus};
use rat_reloc::RelocatableState;
use rat_scrolled::event::ScrollOutcome;
use rat_scrolled::{
  Scroll, ScrollArea, ScrollAreaState, ScrollState, ScrollStyle,
};
  ...
  W: Eq + Clone + Hash,
{
  ...
  pub fn into_buffer(
    ...
    state.vscroll.set_max_offset(
      max_pos.y.saturating_sub(state.widget_area.height) as usize,
    );
    state.hscroll.set_page_len(state.widget_area.width as usize);
    state.hscroll.set_max_offset(
      max_pos.x.saturating_sub(state.widget_area.width) as usize,
    );

    let offset =
      Position::new(state.hscroll.offset as u16, state.vscroll.offset as u16);
  ...
  W: Eq + Hash + Clone,
{
    ...
    WW: Widget,
  {
    ...
    let Some(widget_area) = self.locate_area(self.layout.borrow().widget(idx))
    else {
      return false;
    };
  ...
  pub fn finish(mut self, tgt_buf: &mut Buffer, state: &mut ClipperState<W>) {
    ...
    for y in 0..height {
      ...
      {
        let tgt_0 =
          tgt_buf.index_of(tgt_area.x + off_x0, tgt_area.y + off_y0 + y);
...
impl<W> HandleEvent<crossterm::event::Event, Regular, Outcome>
  for ClipperState<W>
where
  W: Eq + Clone + Hash,
{
...
impl<W> HandleEvent<crossterm::event::Event, MouseOnly, Outcome>
  for ClipperState<W>
where
  W: Eq + Clone + Hash,
{
