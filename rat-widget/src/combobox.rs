...
use crate::choice::{
  Choice, ChoiceClose, ChoiceFocus, ChoicePopup, ChoiceSelect, ChoiceState,
  ChoiceStyle, ChoiceWidget,
};
...
use rat_event::{
  ConsumedEvent, HandleEvent, MouseOnly, Popup, Regular, ct_event,
};
...
impl ComboboxState {
  ...
  pub fn named(name: &str) -> Self {
    ...
    let mut choice = ChoiceState::default();
    let focus =
      focus_cb(FocusFlag::new().with_name(name), text.focus, choice.focus);
    text.focus = focus.clone();
    choice.focus = focus.clone();
...
impl HandleEvent<crossterm::event::Event, Popup, ComboboxOutcome>
  for ComboboxState
{
  fn handle(
    &mut self,
    ...
    let r = if self.is_focused() {
      match event {
        ...
        crossterm::event::Event::Key(_) => {
          match self.text.handle(event, Regular) {
...
impl HandleEvent<crossterm::event::Event, MouseOnly, ComboboxOutcome>
  for ComboboxState
{
  fn handle(
    &mut self,
  ...
  match state.choice.behave_close {
    ...
    ChoiceClose::DoubleClick => match event {
      ct_event!(mouse any for m)
        if state.mouse.doubleclick(state.choice.popup.area, m) =>
      {
