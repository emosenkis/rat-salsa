...
use rat_popup::{
  Placement, PopupCore, PopupCoreState, PopupStyle, fallback_popup_style,
};
...
impl<'a> Choice<'a, usize> {
  ...
  #[inline]
  pub fn auto_items<V: Into<Line<'a>>>(
    self,
...
fn render_choice<T: PartialEq + Clone + Default>(
  ...
  if !state.popup.is_active() {
    ...
    state.popup_scroll.max_offset =
      widget.items.borrow().len().saturating_sub(len as usize);
...
impl<T: PartialEq + Clone + Default>
  HandleEvent<crossterm::event::Event, Popup, ChoiceOutcome>
  for ChoiceState<T>
{
  fn handle(
    &mut self,
...
impl<T: PartialEq + Clone + Default>
  HandleEvent<crossterm::event::Event, MouseOnly, ChoiceOutcome>
  for ChoiceState<T>
{
  fn handle(
    &mut self,
  ...
  match state.behave_close {
    ...
    ChoiceClose::DoubleClick => match event {
      ct_event!(mouse any for m)
        if state.mouse.doubleclick(state.popup.area, m) =>
      {
