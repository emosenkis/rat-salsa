...
impl<'a> View<'a> {
  ...
  pub fn into_buffer(
    ...
    state.layout = if let Some(layout) = self.view_layout {
      ...
      let mut layout =
        Rect::new(0, 0, state.widget_area.width, state.widget_area.height);
    ...
    let r = if self.is_focused() {
      match event {
        ct_event!(keycode press Left) => {
          self.scroll_left(self.hscroll.scroll_by()).into()
        }
        ct_event!(keycode press Right) => {
          self.scroll_right(self.hscroll.scroll_by()).into()
        }
        ct_event!(keycode press Up) => {
          self.scroll_up(self.vscroll.scroll_by()).into()
        }
        ct_event!(keycode press Down) => {
          self.scroll_down(self.vscroll.scroll_by()).into()
        }
        ...
        ct_event!(keycode press ALT-Home) => {
          self.horizontal_scroll_to(0).into()
        }
