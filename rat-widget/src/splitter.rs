...
use crate::splitter::split_impl::{
  get_fill_char, get_join_0, get_join_1, get_mark_0, get_mark_1,
};
use crate::splitter::split_layout::layout_split;
use crate::util::{fill_buf_area, revert_style};
use rat_event::util::MouseFlagsN;
use rat_event::{
  HandleEvent, MouseOnly, Outcome, Regular, ct_event, event_flow,
};
use rat_focus::{FocusBuilder, FocusFlag, HasFocus, Navigation};
use rat_reloc::{
  RelocatableState, relocate_area, relocate_areas, relocate_positions,
};
use ratatui::buffer::Buffer;
...
impl RelocatableState for SplitState {
  fn relocate(&mut self, shift: (i16, i16), clip: Rect) {
    ...
    if !self.relocate_split {
      ...
      relocate_positions(
        self.splitline_mark_position.as_mut_slice(),
        shift,
        clip,
      );
    }
  ...
  fn handle(
    ...
    match event {
      ct_event!(mouse any for m)
        if self.mouse.hover(&self.splitline_areas, m) =>
      {
        Outcome::Changed
      }
      ct_event!(mouse any for m) => {
