    ...
    match tabbed.placement {
      TabPlacement::Top | TabPlacement::Bottom => {
        ...
        for tab in tabbed.tabs.iter() {
          constraints
            .push(Constraint::Length(tab.width() as u16 + 2 + close_width));
        }
  ...
  fn render(
    ...
    for (idx, tab_area) in state.tab_title_areas.iter().copied().enumerate() {
      ...
      match tabbed.placement {
        ...
        TabPlacement::Left => {
          if let Some(cell) =
            buf.cell_mut((tab_area.x + tab_area.width, tab_area.y))
          {
    ...
    if tabbed.closeable {
      for i in 0..state.tab_title_close_areas.len() {
        ...
        if let Some(cell) =
          buf.cell_mut(state.tab_title_close_areas[i].as_position())
        {
