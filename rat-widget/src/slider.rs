  ...
  fn render_knob_str(
    ...
    if is_focused {
      match (self.direction, knob_repeat) {
        ...
        (Direction::Vertical, 1) => {
          map_ref(&self.vertical_knob, Cow::Borrowed("─"))
        }
  ...
  fn layout(&self, area: Rect, state: &mut SliderState<T>) {
    ...
    match self.direction {
      Direction::Horizontal => {
        ...
        state.lower_bound =
          Rect::new(inner.x, inner.y, lower_width, inner.height);
        ...
        state.track = Rect::new(
          state.lower_bound.right(),
          inner.y,
          track_len,
          inner.height,
        );
        ...
        if let Some(knob_pos) =
          state.value.map_range(state.range, (0, state.scale_len))
        {
          state.knob = Rect::new(
            state.track.x + knob_pos,
            inner.y,
            knob_width,
            inner.height,
          )
      ...
      Direction::Vertical => {
        ...
        state.lower_bound =
          Rect::new(inner.x, inner.y, inner.width, lower_height);
        state.upper_bound = Rect::new(
          inner.x,
        ...
        let track_len =
          inner.height.saturating_sub(lower_height + upper_height);
        state.track =
          Rect::new(inner.x, inner.y + lower_height, inner.width, track_len);
        ...
        {
          state.knob =
            Rect::new(inner.x, state.track.y + knob_pos, inner.width, 1)
        } else {
  ...
  u16: MapRange<T>,
{
    ...
    match widget.direction {
      Direction::Horizontal => {
        ...
        let y_offset = match widget.text_align {
          Alignment::Left => 0,
          Alignment::Center => {
            state.lower_bound.height.saturating_sub(lower_height) / 2
          }
          Alignment::Right => {
            state.lower_bound.height.saturating_sub(lower_height)
          }
        };
    ...
    match widget.direction {
      Direction::Horizontal => {
        ...
        let y_offset = match widget.text_align {
          Alignment::Left => 0,
          Alignment::Center => {
            state.upper_bound.height.saturating_sub(upper_height) / 2
          }
          Alignment::Right => {
            state.upper_bound.height.saturating_sub(upper_height)
          }
        };
  ...
  match widget.direction {
    Direction::Horizontal => {
      let knob_str =
        widget.render_knob_str(state.knob.height, state.is_focused());
    ...
    Direction::Vertical => {
      let knob_str =
        widget.render_knob_str(state.knob.width, state.is_focused());
      Line::from(knob_str)
        .alignment(widget.text_align)
        .style(knob_style)
        .render(state.knob, buf);
    ...
    match self.direction {
      Direction::Horizontal => {
        ...
        if x_pos >= self.track.width {
          ...
          x_pos.map_range((0, self.scale_len), self.range)
        {
          self.value = value;
          true
        } else {
      ...
      Direction::Vertical => {
        ...
        if y_pos >= self.track.height {
          ...
          y_pos.map_range((0, self.scale_len), self.range)
        {
          self.value = value;
          true
        } else {
...
impl<T> HandleEvent<crossterm::event::Event, Regular, SliderOutcome>
  for SliderState<T>
where
  T: RangeOp<Step: Copy + Debug>
    + MapRange<u16>
...
impl<T> HandleEvent<crossterm::event::Event, MouseOnly, SliderOutcome>
  for SliderState<T>
where
  T: RangeOp<Step: Copy + Debug>
    + MapRange<u16>
