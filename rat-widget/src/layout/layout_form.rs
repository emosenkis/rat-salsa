...
impl XPositions {
  fn new(page: &Page, column: u16, mirror: bool) -> XPositions {
    ...
    let column_width = layout_width
      .saturating_sub(page.def.column_spacing * n_col_spacers)
      / page.def.columns;
    ...
    match page.def.flex {
      Flex::Legacy => {
        offset =
          border.left + (column_width + page.def.column_spacing) * column;
...
impl Page {
    ...
    W: Eq + Hash + Clone + Debug,
  {
    ...
    let column_width = layout_width
      .saturating_sub(layout.column_spacing * n_col_spacers)
      / layout.columns;
    ...
    let nominal = layout.max_left_padding
      + min_label
      + spacing
      + min_widget
  ...
  where
    W: Eq + Hash + Clone + Debug,
  {
    let (max_label, spacing, max_widget) =
      Self::adjusted_widths(layout, page_size);
  ...
  W: Eq + Hash + Clone + Debug,
{
  ...
  for (idx, widget) in layout.widgets.iter_mut().enumerate() {
    ...
    if !ENDLESS
      && page.y.saturating_add(page.bottom_padding_break) > page.page_end
    {
      // reset to safe-point
      page = saved_page;
  ...
  gen_layout.set_page_count(
    ((page.page_no + page.def.columns) / page.def.columns) as usize,
  );
  gen_layout
}
...
fn page_break<const ENDLESS: bool>(page: &mut Page) {
  ...
  if !ENDLESS {
    page.page_start =
      (page.page_no / page.def.columns).saturating_mul(page.def.height);
    page.page_end = page
      .page_start
  ...
  W: Eq + Hash + Clone + Debug,
{
  ...
  // get areas + advance
  let (label_area, widget_area, advance, gap) =
    areas_and_advance(page, widget, must_fit);
...
fn start_block(page: &mut Page, block: &mut BlockDef) {
  ...
  page.x_pos.container_left =
    page.x_pos.container_left.saturating_add(block.padding.left);
...
fn end_block(page: &mut Page, block: &mut BlockDef) {
  ...
  page.x_pos.container_left =
    page.x_pos.container_left.saturating_sub(block.padding.left);
  page.x_pos.container_right = page
    .x_pos
    .container_right
