...
use rat_event::{
  Dialog, HandleEvent, MouseOnly, Outcome, Regular, ct_event, event_flow,
};
use rat_focus::{
  Focus, FocusBuilder, FocusFlag, HasFocus, Navigation, on_lost,
};
...
use ratatui::layout::{
  Alignment, Constraint, Direction, Flex, Layout, Position, Rect, Size,
};
  ...
  fn render(self, area: Rect, buf: &mut Buffer, state: &mut Self::State) {
    ...
    match state.mode {
      ...
      Mode::OpenMany => {
        render_open_many(
          &self,
          layout.widget_for(DialogItem::Content),
          buf,
          state,
        );
      }
...
impl HandleEvent<Event, Dialog, Result<FileOutcome, io::Error>>
  for FileDialogState
{
  fn handle(
    &mut self,
  ...
  if state.file_state.is_focused() {
    ...
    event_flow!({
      match &mut state.file_state {
        ...
        FileStateMode::OpenMany(st) => {
          handle_nav_many(st, &state.files, event)?
        }
