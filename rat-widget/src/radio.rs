...
impl<'a> Radio<'a, usize> {
  ...
  #[inline]
  pub fn auto_items<V: Into<Text<'a>>>(
    mut self,
...
impl<T> HandleEvent<crossterm::event::Event, Regular, RadioOutcome>
  for RadioState<T>
where
  T: PartialEq + Clone + Default,
{
...
impl<T> HandleEvent<crossterm::event::Event, MouseOnly, RadioOutcome>
  for RadioState<T>
where
  T: PartialEq + Clone + Default,
{
