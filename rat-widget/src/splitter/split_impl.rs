  ...
  let s: Option<&str> = if let Some(join_0) = widget.join_0 {
    match (widget.direction, join_0, widget.split_type) {
      (
        ...
        FullPlain | FullQuadrantInside | FullQuadrantOutside | FullEmpty
        | Scroll,
      ) => Some("\u{252C}"),
      (
        Vertical,
      ...
      (Vertical, BorderType::Plain | BorderType::Rounded, FullThick) => {
        Some("\u{251D}")
      }
      ...
      (
        Horizontal,
        BorderType::Double,
        FullPlain | FullThick | FullQuadrantInside | FullQuadrantOutside
        | FullEmpty | Scroll,
      ) => Some("\u{2564}"),
      (
        Vertical,
...
pub(super) fn get_join_1<'a>(
  ...
  s.map(|s| {
    (
      match widget.direction {
        Horizontal => {
          Position::new(split_area.x, state.area.y + state.area.height - 1)
        }
        Vertical => {
          Position::new(state.area.x + state.area.width - 1, split_area.y)
        }
      },
      s,
    )
