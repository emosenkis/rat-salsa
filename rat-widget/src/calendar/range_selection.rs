...
use crate::calendar::{
  CalendarSelection, MonthState, first_day_of_month, is_first_day_of_month,
  is_last_day_of_month, is_same_month, is_same_week, last_day_of_month,
};
...
impl HandleEvent<crossterm::event::Event, Regular, CalOutcome>
  for MonthState<RangeSelection>
{
  fn handle(
    &mut self,
...
impl HandleEvent<crossterm::event::Event, MouseOnly, CalOutcome>
  for MonthState<RangeSelection>
{
  fn handle(
    &mut self,
      ...
      if self.is_focused() {
        match event {
          ct_event!(key press CONTROL-'a') => {
            if self
              .select_month(self.months[self.primary_idx()].start_date(), false)
            {
              CalOutcome::Selected
            } else {
              CalOutcome::Continue
