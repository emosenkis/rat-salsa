  ...
  EditableTable::new(
    Table::default()
      .data(TableData1 {
        ...
        fmt1: NumberFormat::news(
          "###,##0.0",
          NumberSymbols::numeric(state.loc),
        )?,
        fmt2: NumberFormat::news(
          "##########",
          NumberSymbols::numeric(state.loc),
        )?,
      })
      ...
      .vscroll(
        Scroll::new().style(ctx.theme.style_style(Style::CONTAINER_ARROW_FG)),
      )
...
impl SampleEditorState {
  fn new(loc: Locale) -> Result<Self, NumberFmtError> {
    Ok(Self {
      ...
      num1: NumberInputState::named("num1")
        .with_loc_pattern("###,##0.0", loc)?,
      num2: NumberInputState::named("num2")
        .with_loc_pattern("##########", loc)?,
      num3: NumberInputState::named("num3")
        .with_loc_pattern("##########", loc)?,
    })
  }
}
