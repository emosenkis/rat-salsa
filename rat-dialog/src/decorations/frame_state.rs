...
impl HandleEvent<crossterm::event::Event, Dialog, WindowFrameOutcome>
  for WindowFrameState
{
  fn handle(
    &mut self,
    ...
    let r = if self.is_focused() {
      match event {
        ...
        ct_event!(keycode press CONTROL-Up) => {
          let mut new_area = self.area;
          if self.area.y != self.limit.y
            || self.area.height != self.limit.height
          {
            new_area.y = self.limit.y;
            new_area.height = self.limit.height;
    ...
    r.or_else(|| match event {
      ct_event!(mouse any for m)
        if self.mouse_close.hover(self.close_area, m) =>
      {
        WindowFrameOutcome::Changed
      }
      ct_event!(mouse down Left for x,y)
        if self.close_area.contains((*x, *y).into()) =>
      {
      ...
      ct_event!(mouse down Left for x,y)
        if self.min_area.contains((*x, *y).into()) =>
      {
        self.flip_minimize();
        WindowFrameOutcome::Changed
      }
      ...
      ct_event!(mouse down Left for x,y)
        if self.max_area.contains((*x, *y).into()) =>
      {
        self.flip_maximize();
        WindowFrameOutcome::Changed
      }
      ...
      ct_event!(mouse any for m)
        if self.mouse_resize.hover(self.resize_area, m) =>
      {
        WindowFrameOutcome::Changed
      }
      ct_event!(mouse any for m)
        if self.mouse_resize.drag(self.resize_area, m) =>
      {
        let mut new_area = self.area;
      ...
      ct_event!(mouse any for m)
        if self.mouse_move.hover(self.move_area, m) =>
      {
        WindowFrameOutcome::Changed
      }
      ct_event!(mouse any for m)
        if self.mouse_move.doubleclick(self.move_area, m) =>
      {
        self.flip_maximize();
        WindowFrameOutcome::Resized
      }
      ...
      ct_event!(mouse down Left for x,y)
        if self.move_area.contains((*x, *y).into()) =>
      {
