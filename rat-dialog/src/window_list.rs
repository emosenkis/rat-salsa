...
struct WindowListCore<Event, Context, Error> {
  ...
  render: RefCell<
    Vec<
      Box<
        dyn Fn(Rect, &mut Buffer, &mut dyn Window<Context>, &mut Context)
          + 'static,
      >,
    >,
  >,
  ...
  Context: 'static,
{
  ...
  pub fn show(
    &self,
    render: impl Fn(Rect, &mut Buffer, &mut dyn Window<Context>, &'_ mut Context)
    + 'static,
    event: impl Fn(
      &Event,
      &mut dyn Window<Context>,
  ...
  pub fn get_mut<'a, S: 'static>(&'a self, n: usize) -> Option<RefMut<'a, S>> {
    ...
    RefMut::filter_map(state, |v| {
      ...
      if let Some(state) = state.as_mut() {
        if let Some(state) =
          (state.as_mut() as &mut dyn Any).downcast_mut::<S>()
        {
          Some(state)
        } else {
          None
...
impl<Event, Context, Error>
  HandleEvent<Event, &mut Context, Result<WindowControl<Event>, Error>>
