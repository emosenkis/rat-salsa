...
pub mod app {
  ...
  fn handle_focus(
    ...
    if let TurboEvent::Event(event) = event {
      ...
      if let ct_event!(keycode press Esc) | ct_event!(keycode press F(10)) =
        event
      {
...
pub mod menu {
  ...
  use rat_widget::menu::{
    MenuBuilder, MenuStructure, Menubar, MenubarPopup, PopupConstraint,
    PopupMenu,
  };
      ...
      TurboEvent::Event(event) => match event {
        ...
        ct_event!(keycode press CONTROL-F(5)) => {
          if let Some(top) = ctx.windows.top::<EditorState>() {
            let editor =
              ctx.windows.get_mut::<EditorState>(top).expect("editor");
        ...
        ct_event!(keycode press F(6)) => {
          return Ok(Control::Event(TurboEvent::WindowNext));
        }
    ...
    match event {
      ...
      TurboEvent::WindowMaximize => {
        try_flow!(if let Some(idx) = ctx.windows.top::<EditorState>() {
          let mut editor =
            ctx.windows.get_mut::<EditorState>(idx).expect("editor");
      ...
      TurboEvent::WindowNext => {
        try_flow!({
          ...
          if len > 0 {
            ...
            let editor =
              ctx.windows.get::<EditorState>(len - 1).expect("editor");
            ctx.focus().focus(&*editor);
            Control::Changed
          } else {
            Control::Continue
