...
pub fn md_make_header(state: &mut TextAreaState, header: u8) -> TextOutcome {
  if md_paragraph(state).is_some() {
    ...
    let md_header =
      parse_md_header(header_byte.start, txt.as_ref()).expect("md header");
...
pub fn md_tab(state: &mut TextAreaState) -> TextOutcome {
  if is_md_table(state) {
    ...
    let indent_x = if item_range.start.y < cursor.y {
      ...
      let item =
        parse_md_item(item_byte.start, item_str.as_ref()).expect("md item");
      ...
      let prev_item =
        parse_md_item(prev_byte.start, prev_str.as_ref()).expect("md item");
...
fn md_paragraph(state: &TextAreaState) -> Option<(Range<usize>, TextRange)> {
  ...
  let para_byte =
    state.styles_at_match(cursor_byte, MDStyle::Paragraph as usize);
...
pub fn md_strong(state: &mut TextAreaState, c: char) -> TextOutcome {
  ...
  if can_remove_4 {
    ...
    if sel.start.y == sel.end.y {
      sel =
        TextRange::new(sel.start, TextPosition::new(sel.end.x - 4, sel.end.y));
    } else {
      sel =
        TextRange::new(sel.start, TextPosition::new(sel.end.x - 2, sel.end.y));
    }
    ...
    if sel.start.y == sel.end.y {
      ...
      sel =
        TextRange::new(sel.start, TextPosition::new(sel.end.x - 1, sel.end.y));
    }
  }
  if can_insert {
    let cc = match c {
