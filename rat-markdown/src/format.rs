use crate::parser::{
  MDItem, parse_md_block_quote, parse_md_item, parse_md_link_ref, parse_md_row,
};
use crate::styles::MDStyle;
use crate::util::str_line_len;
use pulldown_cmark::{
  BlockQuoteKind, CodeBlockKind, CowStr, Event, HeadingLevel, OffsetIter,
...
impl<'a> Reformat<'a> {
  ...
  fn indent_prefix(
    ...
    if last_end < start_byte {
      ...
      if let Some(link_ref) =
        parse_md_link_ref(last_end, &self.txt[last_end..start_byte])
      {
  ...
  fn word_cursor(&self, word: &str) -> Option<usize> {
    ...
    let word_range =
      if word_ptr >= txt_ptr && word_ptr < txt_ptr + self.txt.len() {
...
pub fn reformat(
  ...
  if last_r.end < txt.len() {
    // link refs are parsed irregularly.
    if let Some(link_ref) =
      parse_md_link_ref(last_r.end, &arg.txt[last_r.end..txt.len()])
    {
...
fn reformat_list<'a>(
  ...
  // list prefix
  let first_item =
    parse_md_item(list_range.start, &arg.txt[list_range]).expect("md item");
...
fn reformat_blockquote<'a>(
  ...
  let block_quote =
    parse_md_block_quote(block_range.start, &arg.txt[block_range.clone()])
      .expect("block quote");
...
fn wrap_words<'a>(
  ...
  let words = mem::take(&mut arg.words);
  let wrapped = WrapAlgorithm::OptimalFit(Penalties::default())
    .wrap(&words, widths.as_slice());
