...
impl HandleEvent<crossterm::event::Event, MarkDown, TextOutcome>
  for TextAreaState
{
  fn handle(
    &mut self,
    ...
    if self.is_focused() {
      flow!(match event {
        ...
        ct_event!(key press ALT-'c') =>
          md_surround(self, "```\n", None, "\n```", Some(0)),
        ct_event!(key press ALT-'i') =>
          md_surround(self, "![", None, "]()", Some(2)),
        ct_event!(key press ALT-'l') =>
          md_surround(self, "[", None, "]()", Some(2)),
        ct_event!(key press ALT-'k') =>
          md_surround(self, "[", None, "][]", Some(2)),
        ct_event!(key press ALT-'r') =>
          md_surround(self, "[", Some(1), "]: ", None),
        ct_event!(key press ALT-'f') =>
          md_surround(self, "[^1]", Some(4), "", None),
