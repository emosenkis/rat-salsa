...
use rat_text::{
  TextError, TextFocusGained, TextFocusLost, TextStyle, TextTab,
  derive_text_widget_state, upos_type,
};
...
fn render(
  ...
  let mode_label =
    Rect::new(mode_area.x, mode_area.y + mode_area.height / 2, 4, 1);
...
impl ColorInputState {
  ...
  fn parse_value(&self) -> (f32, f32, f32) {
    match self.mode {
      ...
      Mode::HEX => {
        let v =
          u32::from_str_radix(self.widget.section_text(1), 16).expect("hex");
    ...
    if self.is_focused() {
      ...
      if !self.disable_modes {
        flow!(match event {
          ...
          | ct_event!(keycode press PageDown) => self.prev_mode().into(),
          _ => TextOutcome::Continue,
        });
      }
    }
  ...
  if state.is_focused() {
    match event {
      ct_event!(scroll ALT down for x,y)
        if state.mode_area.contains((*x, *y).into()) =>
      {
      ...
      ct_event!(scroll down for x,y)
        if state.widget.area.contains((*x, *y).into()) =>
      {
        let rx = state
          .widget
