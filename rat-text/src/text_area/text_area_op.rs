...
pub fn insert_char(state: &mut TextAreaState, c: char) -> bool {
  ...
  if pos.x == 0
    && pos.y != 0
    && (pos.y == state.len_lines()
      || pos.y == state.len_lines().saturating_sub(1))
  ...
  if state.has_selection() {
    ...
    if till_line_start {
      ...
      if end != cursor {
        ...
        let start =
          prev_word_start(&state.value, cursor).expect("valid_cursor");
        state
          .value
