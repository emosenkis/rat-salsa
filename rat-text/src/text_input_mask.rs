...
use crate::{
  Grapheme, HasScreenCursor, TextError, TextFocusGained, TextFocusLost,
  TextPosition, TextRange, TextStyle, TextTab, ipos_type, upos_type,
};
...
impl MaskedInputState {
  ...
  fn parse_mask(mask_str: &str) -> Result<Vec<MaskToken>, fmt::Error> {
    ...
    for m in mask_str.graphemes(true).chain(once("")) {
      ...
      if matches!(mask, Mask::Separator(_))
        || mask.section() != last_mask.section()
      {
      ...
      if matches!(mask, Mask::Separator(_))
        || mask.sub_section() != last_mask.sub_section()
      {
...
impl MaskedInputState {
  ...
  /// Get the default cursor position for the next editable section.
  pub fn next_section_range(
    &self,
    cursor: upos_type,
  ) -> Option<Range<upos_type>> {
    if cursor as usize >= self.mask.len() {
      return None;
    }
...
impl MaskedInputState {
  ...
  #[inline]
  pub fn section_value<T: FromStr>(
    &self,
    section: u16,
  ...
  pub fn try_byte_range(
    &self,
    bytes: Range<usize>,
  ) -> Result<Range<upos_type>, TextError> {
...
impl MaskedInputState {
  ...
  pub fn select_prev_section(&mut self) -> bool {
    ...
    if let Some(next) =
      self.prev_section_range(selection.start.saturating_sub(1))
    {
      if !next.is_empty() {
        self.set_selection(next.start, next.end)
      } else {
...
impl MaskedInputState {
  fn glyphs2(&self) -> impl Iterator<Item = Glyph2<'_>> {
    ...
    let mut it =
      GlyphIter2::new(TextPosition::new(0, 0), 0, iter, Default::default());
  ...
  fn handle(
    ...
    let mut r = if self.is_focused() {
      match event {
        ...
        | ct_event!(keycode press SHIFT-Delete) => {
          clear_overwrite(self);
