...
use crate::range_map::{
  RangeMap, expand_range_by, ranges_intersect, shrink_range_by,
};
use crate::text_store::TextStore;
use crate::undo_buffer::{
  StyleChange, TextPositionChange, UndoBuffer, UndoEntry, UndoOp,
};
  ...
  pub fn replay_log(&mut self, replay: &[UndoEntry]) {
    for replay_entry in replay {
      match &replay_entry.operation {
        ...
        UndoOp::InsertChar { bytes, txt, .. }
        | UndoOp::InsertStr { bytes, txt, .. } => {
...
impl<Store: TextStore + Default> TextCore<Store> {
  ...
  pub fn grapheme_at(
    ...
    let range_bytes =
      self.bytes_at_range(TextRange::new(pos, (pos.x + 1, pos.y)))?;
  ...
  pub fn text_graphemes(
    ...
    let range_bytes =
      self.bytes_at_range(TextRange::new((0, 0), (cols, rows)))?;
    let pos_byte = self.byte_at(pos)?.start;
...
impl<Store: TextStore + Default> TextCore<Store> {
  ...
  fn _remove_range(
    ...
    let (old_text, (_removed_range, removed_bytes)) =
