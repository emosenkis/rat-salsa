...
impl<'a> Glyph2<'a> {
  ...
  pub fn contains_screen_x(&self, screen_x: u16) -> bool {
    if screen_x as upos_type >= self.screen_pos.0 {
      if (screen_x as upos_type)
        < self.screen_pos.0 + self.screen_width as upos_type
      {
        return true;
      }
      if self.line_break {
...
fn prepare_word_wrap<'a, Graphemes>(
  ...
  Graphemes: Iterator<Item = Grapheme<'a>> + SkipLine + Clone,
{
  ...
  loop {
      ...
      && (glyph.glyph == " " || glyph.glyph == "-" || glyph.hidden_break)
    {
      ...
      (space_pos, space_screen_x, space_byte) = (None, None, None);
    } else if glyph.screen_pos.0 + glyph.screen_width
      >= glyphs.true_right_margin()
    {
...
fn prepare_hard_wrap<'a, Graphemes>(
  glyphs: &mut GlyphIter2<'a, Graphemes>,
) -> Result<(), TextError>
where
  Graphemes: Iterator<Item = Grapheme<'a>> + SkipLine + Clone,
  ...
  Graphemes: SkipLine + Iterator<Item = Grapheme<'a>> + Clone,
{
  ...
  if glyph.line_break {
    ...
    if glyph.screen_pos.0 <= glyphs.true_right_margin() + 1 {
      glyph.screen_pos.0 =
        glyph.screen_pos.0.saturating_sub(glyphs.left_margin);
      glyph.validate();
    ...
    && glyph.screen_pos.0 + glyph.screen_width > glyphs.left_margin
  {
    ...
    glyph.screen_width =
      glyphs.next_screen_pos.0 + glyph.screen_width - glyphs.left_margin;
      ...
      > glyphs.true_right_margin()
  {
    ...
    glyph.screen_width =
      glyphs.next_screen_pos.0 + glyph.screen_width - glyphs.right_margin;
...
fn remap_glyph(
  ...
  if glyph.glyph == "\n" || glyph.glyph == "\r\n" {
    ...
    glyph.line_break = false;
    glyph.screen_width =
      unicode_display_width::width(&glyph.glyph) as upos_type;
