...
impl TextRope {
  ...
  fn normalize(
    &self,
    pos: TextPosition,
  ) -> Result<(TextPosition, usize), TextError> {
...
impl TextStore for TextRope {
  ...
  fn insert_char(
    ...
    let mut it_gr = RopeGraphemes::new_offset(0, self.text.slice(..), pos_byte)
      .expect("valid_bytes");
    let prev = it_gr.prev();
    it_gr.next();
    let next = it_gr.next();
...
impl From<ropey::Error> for TextError {
  fn from(err: ropey::Error) -> Self {
    use ropey::Error;
    match err {
      Error::ByteIndexOutOfBounds(i, l) => {
        TextError::ByteIndexOutOfBounds(i, l)
      }
      Error::CharIndexOutOfBounds(i, l) => {
        TextError::CharIndexOutOfBounds(i, l)
      }
      ...
      Error::ByteIndexNotCharBoundary(i) => {
        TextError::ByteIndexNotCharBoundary(i)
      }
      Error::ByteRangeNotCharBoundary(s, e) => {
        TextError::ByteRangeNotCharBoundary(s, e)
      }
      Error::ByteRangeInvalid(s, e) => TextError::ByteRangeInvalid(s, e),
      Error::CharRangeInvalid(s, e) => TextError::CharRangeInvalid(s, e),
      Error::ByteRangeOutOfBounds(s, e, l) => {
        TextError::ByteRangeOutOfBounds(s, e, l)
      }
      Error::CharRangeOutOfBounds(s, e, l) => {
        TextError::CharRangeOutOfBounds(s, e, l)
      }
      _ => {
        unreachable!("{:?}", err)
      }
