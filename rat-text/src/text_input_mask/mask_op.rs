  ...
  if range.start >= mask.sub_start && range.end <= mask.sub_end {
    if mask.right.is_rtol() {
      ...
      state
        ...
        .expect("valid_range");
      let fill_before = &state.mask
        [mask.sub_start as usize..mask.sub_start as usize + range.len()];
      state
        .value
        .insert_str(
  ...
  loop {
    ...
    if mask.sub_start < range.start {
      ...
      if mask.right.is_rtol() {
        ...
        let fill_after =
          &state.mask[range.start as usize..mask.sub_end as usize];
        state
          .value
          .insert_str(
      ...
      if mask.right.is_rtol() {
        ...
        let fill_before =
          &state.mask[mask.sub_start as usize..range.end as usize];
        state
          .value
          .insert_str(
            TextPosition::new(mask.sub_start, 0),
        ...
        let fill_after =
          &state.mask[(mask.sub_end - len) as usize..mask.sub_end as usize];
        state
          .value
          .insert_str(
      ...
      let sec_range =
        &state.mask[mask.sub_start as usize..mask.sub_end as usize];
      state
        .value
        .insert_str(
          TextPosition::new(mask.sub_start, 0),
...
fn reformat(
  ...
  if mask[section.start as usize].right.is_rtol() {
    ...
    core
      .remove_char_range(TextRange::new((section.start, 0), (section.end, 0)))
      .expect("valid_range");
    core
      .insert_str(TextPosition::new(section.start, 0), &out)
    ...
    if sec_str != str_new {
      core
        ...
        .expect("valid_range");
      core
        .insert_str(TextPosition::new(section.start, 0), &str_new)
        .expect("valid_position");
...
fn insert_ltor(state: &mut MaskedInputState, c: char) -> bool {
  ...
  let g = state
    ...
    .expect("valid_cursor")
    .expect("mask");
  if mask.right.can_overwrite(g.grapheme())
    && is_valid_char(state, &mask.right, c)
  {
...
pub fn remove_next(state: &mut MaskedInputState) {
  ...
  if right.right.is_rtol() {
    ...
    let sec_str = state
      ...
      .expect("valid_range");
    let sec_mask =
      &state.mask[right.sub_start as usize..right.sub_end as usize];
  ...
  if left.right.is_rtol() {
    ...
    let sec_empty = if left.right.is_rtol() {
      let sec_str = state
        ...
        .expect("valid_range");
      let sec_mask =
        &state.mask[left.sub_start as usize..left.sub_end as usize];
