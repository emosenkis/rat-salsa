...
use crate::{
  HasScreenCursor, TextError, TextPosition, TextRange, TextStyle, ipos_type,
  upos_type,
};
...
impl<'a> TextArea<'a> {
  ...
  /// Use [TextAreaState::add_style()] to refer a text range to
  /// one of these styles.
  pub fn text_style<T: IntoIterator<Item = Style>>(
    mut self,
    styles: T,
  ) -> Self {
  ...
  /// Use [TextAreaState::add_style()] to refer a text range to
  /// one of these styles.
  pub fn text_style_map<T: Into<Style>>(
    mut self,
    styles: HashMap<usize, T>,
  ) -> Self {
    ...
    .expect("valid_offset")
  {
    ...
    if g.screen_width() > 0 {
      ...
      if let Some(cell) = buf
        .cell_mut((state.inner.x + screen_pos.0, state.inner.y + screen_pos.1))
      {
...
impl TextAreaState {
  ...
  pub fn styles(
    &self,
  ) -> Option<impl Iterator<Item = (Range<usize>, usize)> + '_> {
    self.value.styles()
  }
}
...
impl TextAreaState {
  ...
  ) -> (TextWrap2, upos_type, upos_type, upos_type) {
    match self.text_wrap {
      TextWrap::Shift => (
        TextWrap2::Shift,
        shift_left,
  ...
  fn glyphs2(
    ...
    GlyphIter2<'_, <TextRope as TextStore>::GraphemeIter<'_>>,
    TextError,
  > {
    let (text_wrap, left_margin, right_margin, word_margin) =
      self.text_wrap_2(shift_left);
  ...
  fn stc_fill_screen_cache(&self, scr: (upos_type, upos_type, upos_type)) {
    let y2 = scr.1 + scr.2;
    self
      .fill_cache(0, scr.0, scr.1..min(y2, self.len_lines()))
      .expect("valid-rows");
  }
...
impl TextAreaState {
      ...
      TextWrap::Hard | TextWrap::Word(_) => {
        ...
        if scr_pos.1 < 0 {
          ...
          let n_start_pos = 'f: {
            ...
            for (_break_pos, cache) in
              line_break.range(start_range..end_range).rev()
            {
              if nrows == 0 {
          ...
          let n_start_pos = if scr_pos.1 == 0 {
            ...
            'f: {
              ...
              for (_break_pos, cache) in
                text_range.range(start_range..end_range)
              {
                if nrows == 0 {
                  break 'f cache.start_pos;
                }
  ...
  pub fn pos_to_screen(
    ...
    if scr_pos.0 + self.inner.x as i16 > 0
      && scr_pos.1 + self.inner.y as i16 > 0
    {
      Some((
  ...
  pub fn pos_to_line_start(
    ...
    match self.text_wrap {
      ...
      TextWrap::Hard | TextWrap::Word(_) => {
        self
          .fill_cache(0, 0, pos.y..min(pos.y + 1, self.len_lines()))
          .expect("valid-row");
  ...
  /// They may be negative too, this allows setting the cursor
  /// to a position that is currently scrolled away.
  pub fn set_screen_cursor_words(
    &mut self,
    cursor: (i16, i16),
    extend_selection: bool,
  ) -> bool {
      ...
      match self.text_wrap {
        ...
        TextWrap::Hard | TextWrap::Word(_) => {
            ...
            self.stc_screen_row(scr, TextPosition::new(sub_row_offset, oy))
          {
            if let Some(pos_row) = self.stc_screen_row(scr, pos) {
              if pos_row < off_row && pos_row >= off_row.saturating_sub(page) {
                ...
                let (nsub_row_offset, noy) =
                  self.stc_sub_row_offset(scr, noff_row);
                self.set_offset((0, noy));
                self.set_sub_row_offset(nsub_row_offset);
                break 'f;
              } else if pos_row >= off_row + page
                && pos_row < off_row + 2 * page
              {
          ...
          if let Some(alt_scr_row) = self.stc_screen_row(alt_scr, pos) {
            ...
            let (nsub_row_offset, noy) =
              self.stc_sub_row_offset(alt_scr, noff_row);
            self.set_offset((0, noy));
            self.set_sub_row_offset(nsub_row_offset);
          } else {
...
impl HandleEvent<crossterm::event::Event, Regular, TextOutcome>
  for TextAreaState
{
  fn handle(
    &mut self,
    ...
    let mut r = if self.is_focused() {
      match event {
        ...
        | ct_event!(keycode press SHIFT-Backspace) => {
          tc(self.delete_prev_char())
        }
        ct_event!(keycode press Delete)
        | ct_event!(keycode press SHIFT-Delete) => tc(self.delete_next_char()),
...
impl HandleEvent<crossterm::event::Event, ReadOnly, TextOutcome>
  for TextAreaState
{
  fn handle(
    &mut self,
    ...
    let mut r = if self.is_focused() {
      match event {
        ...
        ct_event!(keycode press CONTROL-PageUp) => {
          self.move_to_screen_start(false).into()
        }
        ct_event!(keycode press CONTROL-PageDown) => {
          self.move_to_screen_end(false).into()
        }
        ct_event!(keycode press CONTROL-Home) => {
          self.move_to_start(false).into()
        }
        ...
        ct_event!(keycode press CONTROL_SHIFT-Home) => {
          self.move_to_start(true).into()
        }
        ct_event!(keycode press CONTROL_SHIFT-End) => {
          self.move_to_end(true).into()
        }
...
impl HandleEvent<crossterm::event::Event, MouseOnly, TextOutcome>
  for TextAreaState
{
  fn handle(
    &mut self,
    ...
    flow!(match event {
      ...
      ct_event!(mouse any for m)
        if self.mouse.drag2(self.inner, m, KeyModifiers::ALT) =>
      {
        let cx = m.column as i16 - self.inner.x as i16;
        let cy = m.row as i16 - self.inner.y as i16;
