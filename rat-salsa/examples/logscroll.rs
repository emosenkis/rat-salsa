...
use rat_widget::text::clipboard::{
  Clipboard, ClipboardError, set_global_clipboard,
};
...
fn load_config() -> Result<LogScrollConfig, Error> {
  if let Some(config) = config_dir() {
    ...
    if config.exists() {
      ...
      let split_pos: u16 =
        ini.get("default", "split").unwrap_or("25".into()).parse()?;
...
mod logscroll {
  ...
  use rat_widget::event::{
    HandleEvent, Outcome, ReadOnly, Regular, TableOutcome, TextOutcome,
    ct_event, try_flow,
  };
  ...
  pub fn render(
    ...
    let find_area =
      Rect::new(l2[1].x, l2[1].y, l2[1].width.saturating_sub(1), 1);
    Paired::new(
    ...
    let matches_area =
      Rect::new(l2[3].x, l2[3].y, l2[3].width.saturating_sub(1), 1);
