...
use rat_widget::event::{
  Dialog, DoubleClick, DoubleClickOutcome, HandleEvent, MenuOutcome, Popup,
  ReadOnly, Regular, TableOutcome, ct_event, try_flow,
};
...
fn show_file(
  ...
  if let Some(file) = file {
    if file.is_file() {
      ...
      let (cancel_show, _) =
        ctx.spawn_ext(move |can, snd| match fs::read(&file) {
          Ok(data) => {
