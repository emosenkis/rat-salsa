...
pub fn event(
  ...
  let mut r = match event {
    TurboEvent::Event(event) => {
      ...
      r = r.or_else(|| {
        ctx
          .set_focus(FocusBuilder::rebuild_for(&state.turbo, ctx.take_focus()));
...
pub mod turbo {
  ...
  use rat_widget::event::{
    HandleEvent, MenuOutcome, Popup, ct_event, try_flow,
  };
  use rat_widget::focus::impl_has_focus;
  use rat_widget::menu::{
    MenuBuilder, MenuStructure, Menubar, MenubarState, PopupConstraint,
