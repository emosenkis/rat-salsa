...
/// __ Not builtin __
///
/// This feature is not built into rat-salsa.
/// Rather you add it to your global state when needed.
/// You need to call [render] at the end of rendering
/// and [handle] at the beginning of event-handling.
///
...
struct DialogStackCore<Event, Context, Error> {
  len: Cell<usize>,
  render: RefCell<
    Vec<Box<dyn Fn(Rect, &mut Buffer, &mut dyn Any, &mut Context) + 'static>>,
  >,
  event: RefCell<
    Vec<
      Box<
...
impl<Event, Context, Error>
  HandleEvent<Event, &mut Context, Result<Control<Event>, Error>>
  ...
  fn handle(
    ...
    for n in (0..self.core.len.get()).rev() {
      ...
      let event_fn = mem::replace(
        ...
        Box::new(|_, _, _| Ok(Control::Continue)),
      );
