...
use rat_theme4::{
  RatWidgetColor, create_palette_theme, load_palette, store_palette,
};
...
pub const fn color_array_no_text() -> [Colors; Colors::LEN - 2] {
  ...
  [
    Primary, Secondary, White, Black, Gray, Red, Orange, Yellow, LimeGreen,
    Green, BlueGreen, Cyan, Blue, DeepBlue, Purple, Magenta, RedPink,
  ]
}
...
pub const fn color_array() -> [Colors; Colors::LEN] {
  ...
  [
    ...
    Yellow, LimeGreen, Green, BlueGreen, Cyan, Blue, DeepBlue, Purple, Magenta,
    RedPink,
  ]
}
...
pub fn load_patch(
  ...
  ctx.show_theme =
    create_palette_theme(pal).unwrap_or_else(|p| create_fallback(p));
