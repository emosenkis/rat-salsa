...
use rat_widget::event::{
  ChoiceOutcome, HandleEvent, MouseOnly, Outcome, Popup, Regular, TextOutcome,
  event_flow,
};
...
impl HasScreenCursor for PaletteEdit {
  fn screen_cursor(&self) -> Option<(u16, u16)> {
    None
      .or(self.theme_name.screen_cursor())
...
pub fn event(
  ...
  let r = 'f: {
    ...
    for c in color_array() {
      event_flow!(
        break 'f handle_color(
          event,
          &mut state.color[c as usize].0,
          &mut mode_change
        )
      );
      event_flow!(
