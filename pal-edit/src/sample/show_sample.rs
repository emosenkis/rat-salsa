...
use crate::sample::{
  sample_custom, sample_data_input, sample_list, sample_readability,
  sample_split, sample_table,
};
use anyhow::Error;
...
pub fn render(
  ...
  match state.tabs.selected() {
    ...
    Some(1) => {
      sample_readability::render(
        state.tabs.widget_area,
        buf,
        &mut state.readability,
        ctx,
      )?;
    }
    Some(2) => {
